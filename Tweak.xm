...
static NSString * const kWALGMenuInstalledKey = @"kWALGMenuInstalledKey";
static NSString * const kWALGOverridesKey = @"com.darthplagueiswise.walg.overrides";

static NSMutableDictionary *WALGLoadOverrides(void) {
    NSDictionary *stored = [[NSUserDefaults standardUserDefaults] objectForKey:kWALGOverridesKey];
...
static void WALGSetOverride(NSString *key, BOOL enabled) {
    NSMutableDictionary *stored = WALGLoadOverrides();
    stored[key] = @(enabled);
    [[NSUserDefaults standardUserDefaults] setObject:stored forKey:kWALGOverridesKey];
...
static void WALGWriteExperimentDefaults(void) {
    if (!WALGOverrideEnabled(@"Bootstrap.WriteDefaults")) return;
...
static void WALGEnableOverrideHelper(void) {
    if (!WALGOverrideEnabled(@"Bootstrap.EnableOverrideHelper")) return;
    ...
    if (![object_getClass(overrideClass) respondsToSelector:sharedInstanceSelector]) return;
...
#pragma clang diagnostic pop
    ...
    if (![sharedInstance respondsToSelector:setEnabledSelector]) return;
...
static UIViewController *WALGTopViewController(void) {
    UIWindow *window = nil;
...
static void WALGPresentMenu(void) {
    UIViewController *top = WALGTopViewController();
    if (!top) return;
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"LiquidGlass Experiments" message:@"Tap to toggle overrides" preferredStyle:UIAlertControllerStyleActionSheet];
    for (NSString *key in WALGHookKeys()) {
        BOOL enabled = WALGOverrideEnabled(key);
        ...
        [alert addAction:[UIAlertAction actionWithTitle:title style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
            WALGSetOverride(key, !enabled);
            WALGWriteExperimentDefaults();
            WALGEnableOverrideHelper();
        }]];
    }
...
static void WALGHandleLongPress(UIGestureRecognizer *gesture) {
...
- (void)didMoveToWindow {
    %orig;
    NSNumber *installed = objc_getAssociatedObject(self, (__bridge const void *)(kWALGMenuInstalledKey));
    if ([installed boolValue]) return;
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:[NSBlockOperation blockOperationWithBlock:^{}] action:nil];
    [longPress addTarget:[NSValue valueWithPointer:&WALGHandleLongPress] action:@selector(nonexistent)];
    [self addGestureRecognizer:longPress];
    objc_setAssociatedObject(self, (__bridge const void *)(kWALGMenuInstalledKey), @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
%end
